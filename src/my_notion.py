...
def check_existing_id(database_id, arxiv_id):
    results = read_notion_database(notion, database_id)
    for page in results["results"]:
        id_property = page["properties"].get("arXiv-ID", {})
        if id_property and id_property["rich_text"]:
            page_id = id_property["rich_text"][0]["text"]["content"]
            if page_id == arxiv_id:
                return True
    return False
...
def insert_arXiv_database(database_id, page_info):
    ...
    try:
        notion.pages.create(
            ...
            properties={
                "arXiv-ID": create_text_content(arxiv_id),
                ...
                "LLM-summary": create_text_content(page_info["LLM-summary"]),
...
if __name__ == "__main__":
    ...
    page_info["LLM-summary"] = "ここにサマリーが入る"
